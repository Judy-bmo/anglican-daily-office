...
### `public/data/canticles.json`

독서 후에 부르는 송가입니다. 181–189쪽의 열 편에 예식문 안의 복음송가 셋(즈가리야·
성모 마리아·시므온)을 더해 열세 편입니다. `table`은 180–181쪽의 배정표로, 요일과
절기에 따라 어느 송가를 쓰는지 적어 둔 것입니다. 첫째는 1독서 뒤, 둘째는 2독서
뒤에 부르고, 독서가 하나면 첫째만 합니다.

```jsonc
{ "canticles": [
    { "name": "모세송가", "latin": "Cantemus Domino", "ref": "출애 15:1-6, 11-13, 17-18",
      "rubric": "특별히 부활절기에 적합하다.",
      "verses": [{ "n": "1", "text": "그지없이 높으신 주님을 …" }] }],
  "table": [
    { "day": "sunday", "office": "morning", "season": "advent",
      "canticles": ["이사야 셋째 송가", "즈가리야송가"] }] }
```

### `public/data/collects.json`

교회력에 따른 오늘의 본기도입니다. 예식문은 「교회력에 따른 오늘의 본기도를 드린다」고만
적어 두고, 기도문은 41–84쪽에 따로 실려 있습니다. `key`가 있으면 절기와 주차로,
없으면 이름으로 교회력과 이어 붙입니다.

```jsonc
{ "collects": [
    { "season": "연중시기", "day": "연중 22주일(8월 28일과 9월 3일 사이의 주일)",
      "name": "연중 22주일", "page": 75, "rubric": null,
      "key": { "season": "ordinary", "week": 22 },
      "cycles": [{ "cycle": "가", "texts": ["주 하느님, 성자 예수께서는 …"] },
                 { "cycle": "주간", "texts": ["능력의 하느님, …"] }] }] }
```
